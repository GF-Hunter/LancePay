...
function makeReq(token: string | null = 'Bearer valid-token') {
  ...
  return new NextRequest('http://localhost/api/routes-b/automations/actions', { headers })
}
...
describe('GET /api/routes-b/automations/actions', () => {
  ...
  it('returns the list of available actions on the happy path', async () => {
    const res = await GET(makeReq())
  ...
  it('returns 500 when an unexpected error occurs', async () => {
    ...
    const json = await res.json()
    expect(json.error).toBe('Failed to fetch automation actions')
  })
})

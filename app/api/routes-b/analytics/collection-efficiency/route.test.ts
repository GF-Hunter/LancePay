...
const BASE_URL = 'http://localhost/api/routes-b/analytics/collection-efficiency'
...
describe('GET /api/routes-b/analytics/collection-efficiency', () => {
  ...
  it('returns 400 for an invalid days parameter', async () => {
    const res = await GET(makeReq('?days=-5'))
  ...
  it('returns 200 with the collection efficiency ratio on the happy path', async () => {
    const res = await GET(makeReq('?days=30'))
    expect(res.status).toBe(200)
    const json = await res.json()
    expect(json.metric.totalInvoiced).toBe(300)
    expect(json.metric.totalCollected).toBe(250)
    expect(json.metric.collectionEfficiency).toBeCloseTo(0.8333, 3)
  })

  it('returns zero efficiency when there are no invoices', async () => {
    mockInvoiceFindMany.mockResolvedValue([])
    const res = await GET(makeReq())
    expect(res.status).toBe(200)
    const json = await res.json()
    expect(json.metric.collectionEfficiency).toBe(0)
  })
  ...
  it('returns 500 when an unexpected error occurs', async () => {
    ...
    const json = await res.json()
    expect(json.error).toBe('Failed to fetch collection efficiency metric')
  })
})

...
const BASE_URL = 'http://localhost/api/routes-d/integrations/webhooks/inbound'
...
describe('POST /api/routes-d/integrations/webhooks/inbound/[id]/rotate-secret', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns 401 if no authorization header is provided', async () => {
    const { POST } = await import('@/app/api/routes-d/integrations/webhooks/inbound/[id]/rotate-secret/route')
